...
def get_arguments():
    ...
    parser.add_argument("--train_list", type=str,
                        default='list/MOTS/MOTS_train.txt')
    parser.add_argument("--val_list", type=str,
                        default='list/MOTS/MOTS_test.txt')
    parser.add_argument("--snapshot_dir", type=str, default='snapshots/dodnet')
    parser.add_argument("--reload_path", type=str,
                        default='snapshots/dodnet/MOTS_DynConv_checkpoint.pth')
...
def main():
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        # Create model
        model = UNet3D(num_classes=args.num_classes,
        ...
        loss_seg_DICE = loss.DiceLoss4MOTS(
            num_classes=args.num_classes).to(device)
        loss_seg_CE = loss.CELoss4MOTS(
            num_classes=args.num_classes, ignore_index=255).to(device)
        ...
        for epoch in range(args.num_epochs):
            ...
            if (epoch >= 0) and (args.local_rank == 0) and (((epoch % 50 == 0) and (epoch >= 800)) or (epoch % 50 == 0)):
                ...
                if args.FP16:
                    ...
                    torch.save(checkpoint, osp.join(args.snapshot_dir, 'MOTS_DynConv_' +
                               args.snapshot_dir.split('/')[-2] + '_e' + str(epoch) + '.pth'))
                else:
                    torch.save(model.state_dict(), osp.join(args.snapshot_dir, 'MOTS_DynConv_' +
                               args.snapshot_dir.split('/')[-2] + '_e' + str(epoch) + '.pth'))
            ...
            if (epoch >= args.num_epochs - 1) and (args.local_rank == 0):
                ...
                if args.FP16:
                    ...
                    torch.save(checkpoint, osp.join(args.snapshot_dir, 'MOTS_DynConv_' +
                               args.snapshot_dir.split('/')[-2] + '_final_e' + str(epoch) + '.pth'))
                else:
                    torch.save(model.state_dict(), osp.join(args.snapshot_dir, 'MOTS_DynConv_' +
                               args.snapshot_dir.split('/')[-2] + '_final_e' + str(epoch) + '.pth'))
                break
