...
def get_arguments():
    ...
    parser.add_argument("--reload_path", type=str, default='snapshots/dodnet/MOTS_DynConv_checkpoint.pth')
...
def validate(args, input_size, model, ValLoader, num_classes, engine):
    ...
    val_loss = torch.zeros(size=(4, 1)).cuda()  # np.zeros(shape=(7, 1))
    val_Dice = torch.zeros(size=(4, 2)).cuda()  # np.zeros(shape=(7, 2))
    count = torch.zeros(size=(4, 2)).cuda()  # np.zeros(shape=(7, 2))
